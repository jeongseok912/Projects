...
def getData(num):
    try:
        ...
        for doc in cursor:
            ...
            # mongodb에 저장
            print(pcolors.PID + 'PID ' + str(os.getpid()) + pcolors.CRUD + ' (update) ' + pcolors.END + str(doc.get('_id')) + ' - ' + ch.channel_title + ', ' + ch.channel_id + ', ' + str(collection.update({'_id': doc['_id']}, {'$set':
                {
